			...
			<hr style="border: 3px solid #b4c3ff; margin-top:0px; width:85%; margin-left:10px ">
				...
				<script>
					$(function() {
						$('#searchBtn').click(
								function() {
									self.location = "goodslist"
											+ '${gpageMaker.makeQuery(1)}'
											+ "&searchType=pn"
											+ "&keyword="
											+ encodeURIComponent($(
													'#keywordInput').val());
				...
				<form role="form" method="get">
					...
					<c:forEach items="${goodslist }" var="goodslist">
						...
						<article class="col-12 col-md-6 tm-post">
							<input value="<c:out value="${goodslist.pd_bno }" />"
								...
								<div class="tm-post-link-inner">
									...
									<a
										href="/sae_product/read?pd_bno=${goodslist.pd_bno}&pd_type=${goodslist.pd_type}&page=${scri.page}&perPageNum=${scri.perPageNum}
									&searchType=${scri.searchType }&keyword=${srci.keyword }"><img
										style="width: 440px; height: 330px"
										src="/resources/productimg/${goodslist.p_filepath }" /></a>
								</div>

								<h2 class="tm-pt-30 tm-color-primary tm-post-title">
									<c:out value="${goodslist.pd_name}" />
								</h2>
							</a>
							<p class="tm-pt-30">
								<fmt:formatDate value="${goodslist.pd_date}"
									pattern="yyyy-MM-dd" />
							</p>
							<div class="d-flex justify-content-between tm-pt-45">
								<span class="tm-color-primary"> <c:out
										value="${goodslist.pd_price }" /> 원
								</span>
							</div>
							<hr>
					...
					<nav class="tm-paging-nav d-inline-block">
						<ul>
							<c:if test="${gpageMaker.prev}">
								<li><a
									href="goodslist${gpageMaker.makeSearch(gpageMaker.startPage - 1)}"
							...
							<c:forEach begin="${gpageMaker.startPage}"
								...
								<li
									<c:out value = "${gpageMaker.cri.page == idx ? 'class=info' : ''}"/>
									class="tm-paging-item active"><a
									href="goodslist${gpageMaker.makeSearch(idx) }"
									class="mb-2 tm-btn tm-paging-link">${idx}</a></li>
							</c:forEach>

							<c:if test="${gpageMaker.next && gpageMakerendPage > 0 }">
								<li><a
									href="goodslist${gpageMaker.makeSearch(gpageMaker.endPage + 1 )}"
